...
public class XmlParser 
{
  ...
  public void parseXmlString(String xmlText,Document doc , PostgreDB dbdoc) throws ParserConfigurationException, TransformerConfigurationException
  {
      // parse
        ...
        {       
            ...
            saxParser.setContentHandler(new NewHandler(doc ,dbdoc));
            //FROM FILE:
            System.out.println("XML Text : "+ xmlText);
            saxParser.parse (new InputSource(new InputStreamReader(new FileInputStream(new File(xmlText)))));//24meg
...
//             saxParser.parse (new InputSource(new InputStreamReader(new FileInputStream(new File("G:\\Project\\Benchmarks\\SwissProt\\SwissProt.xml")))));//112meg
             // saxParser.parse (new InputSource(new InputStreamReader(new FileInputStream(new File("E:\\Input\\A.xml")))));//24meg
...
//            new InputSource(new InputStreamReader(new FileInputStream(new File("c:..."))));
            System.out.println("A Read Successfully ... \nDone.");
        }
        catch (Exception e)
        {
        	e.printStackTrace();
